...
const Home: React.FC = () => {
  const [activeScene, setActiveScene] = useState<string>(scenes[0].name);
  const [activeSceneData, setActiveSceneData] = useState<Scene | undefined>(scenes[0]);


  // Update activeSceneData whenever activeScene changes
  useEffect(() => {
    const scene = scenes.find(scene => scene.name === activeScene);
    setActiveSceneData(scene);
  }, [activeScene]);

  // Trigger a resize event on mount to fix layout issues
  useEffect(() => {
    setTimeout(() => {
      ...
      <IonContent>
        ...
        <div className="content-container">
          <div className="text-area">
            <IonCard className='model-text' color='primary'>
              <IonCardHeader><strong>{activeSceneData?.label}</strong></IonCardHeader>
              <IonCardContent>{activeSceneData?.jsx}</IonCardContent>
            </IonCard>
          </div>
          <div className="model-container">
            {activeSceneData && (
              <SharedScene
                key={activeSceneData.name}
                modelPath={activeSceneData.modelPath}
                attribution={activeSceneData.attribution}
                cameraPosition={activeSceneData.cameraPosition}
                lightIntensity={activeSceneData.lightIntensity}
              />
            )}
          </div>
        </div>
